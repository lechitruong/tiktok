...
import { GetProp, Modal ,Select,UploadFile,Upload as UploadFileComponent, UploadProps, message } from 'antd';
          ...
          if (info.file.type?.includes('image/')) {
            ...
            if (!src) {
              src = await new Promise((resolve) => {
                ...
                reader.readAsDataURL(info.file as FileType);
            ...
            dispatch(setPostUpload({...postUpload,image : info.fileList[info.fileList.length-1]}))
            setIsModalOpen(false);
            info.fileList = []
          } else {
  ...
  return (
    <div>
        <div className='font-semibold flex gap-3 my-5 mb-2'>
            ...
            <IoIosInformationCircleOutline type='button' fontSize={20} className='my-auto text-gray-400' data-tooltip-target="tooltip-default" />
        </div>
        <div id="tooltip-default" role="tooltip" className="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-950 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
            Select a cover or upload one from your device. An engaging cover can effectively capture viewers’ interest.
            <div className="tooltip-arrow" data-popper-arrow></div>
        </div>
        <div className='w-[150px] h-[200px] relative overflow-hidden rounded-sm shadow-sm' onClick={showModal}>
          {!image &&
            <video muted className='w-full h-full  object-cover object-center' src={videoUrl || ''} />
          }
          {image && 
            <img ref={imageRef} width="100%" height="100%" className='w-full h-full  object-cover object-center' src={image.url}/>
          }
        ...
        <div>
          <p className='my-auto font-semibold mt-5 mb-3'>Who can watch this video</p> 
          <Select
            defaultValue="1"
            style={{ width: 250 }}
        ...
        <div className='my-8'>
          <Button className='max-w-[110px]'>Post</Button>
        </div>
        <Modal 
            ...
            >
              <Dragger {...props} className='w-full flex-1'>
                ...
                <p className="font-bold text-[24px]" >Drap and drop a image file here</p>
                <p className="text-[16px] font-semibold text-[#A8A8A8]">
                  Or <span className='text-[#2b5db9]'>select a file</span>
                </p>
                <p className="text-gray-300 mt-8 text-[14px]" >Supported formats: JPG, JPEG, and PNG.</p>
              </Dragger>
        </Modal>
    </div>              
  )
