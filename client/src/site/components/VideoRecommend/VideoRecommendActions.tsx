...
const VideoRecommendActions = ({post,isFollow,setIsFollow,followUser} : VideoRecommendChildProps) => {
  ...
  const shareVideo = () => {
    navigator.clipboard.writeText(clientURL + "post/" + post.id)
      ...
      .catch(() => {
        showToast.error("Can't share video");
      });
  };
  ...
  return (
    <div className="ms-5 flex flex-col justify-end">
      {!post.isMe ? <div className="mb-4 relative min-w-12 max-w-12 h-12 rounded-full object-cover  object-center">
        <img src={post.posterData.avatarData.url || ""} width="100%" height="100%" alt='User avatar'/>
        <div onClick={followUser} className={
      ...
      <ButtonActionPost onClick={likePost} className='mt-3' icon={<IoHeart fontSize={25} className={clsx('m-auto active:scale-125  transition-all',isLiked && 'text-primary')}/>} count={likes} postId={post.id!}/>
      ...
      <ButtonActionPost onClick={()=> {shareVideo()}} className='mt-3' icon={<FaShare fontSize={25} className='m-auto active:scale-125  transition-all'/>} count={shares} postId={post.id!}/>
    </div>
  )
}
